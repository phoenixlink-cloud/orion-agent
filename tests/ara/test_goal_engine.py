...
class TestTaskDAG:
    def test_create_dag(self):
        dag = TaskDAG(
            ...
            tasks=[
                ...
                Task(task_id="t2", title="B", description="", action_type="write_files", dependencies=["t1"]),
            ],
        )
    ...
    def test_ready_after_completion(self):
        dag = TaskDAG(
            goal="test",
            tasks=[
                Task(task_id="t1", title="A", description="", action_type="read_files", status=TaskStatus.COMPLETED),
                Task(task_id="t2", title="B", description="", action_type="write_files", dependencies=["t1"]),
            ],
        )
        ready = dag.get_ready_tasks()
    ...
    def test_validate_missing_dependency(self):
        dag = TaskDAG(
            goal="test",
            tasks=[
                Task(task_id="t1", title="A", description="", action_type="read_files", dependencies=["missing"]),
            ],
        )
        errors = dag.validate_dag()
        assert len(errors) >= 1
...
class TestMockLLMProvider:
    def test_default_decomposition(self, mock_llm: MockLLMProvider):
        tasks = asyncio.run(
            mock_llm.decompose_goal("any goal")
        )
        assert len(tasks) == 3
        assert tasks[0]["task_id"] == "task-1"

    def test_custom_responses(self):
        custom = MockLLMProvider(responses={
            "custom goal": [{"task_id": "c1", "title": "Custom", "description": "", "action_type": "read_files"}],
        })
        tasks = asyncio.run(
            custom.decompose_goal("custom goal")
        )
        assert len(tasks) == 1
        assert tasks[0]["task_id"] == "c1"

    def test_replan(self, mock_llm: MockLLMProvider):
        remaining = [{"task_id": "r1", "title": "Remaining", "description": "", "action_type": "write_files"}]
        result = asyncio.run(
            mock_llm.replan("goal", remaining=remaining)
        )
        assert len(result) >= 1
...
class TestGoalEngine:
    def test_decompose(self, engine: GoalEngine):
        dag = asyncio.run(
            engine.decompose("Add tests for auth")
        )
        assert dag.total_tasks == 3
        assert dag.goal == "Add tests for auth"

    def test_decompose_invalid_raises(self):
        bad_llm = MockLLMProvider(responses={
            "bad": [{"task_id": "t1", "title": "A", "description": "", "action_type": "x", "dependencies": ["missing"]}],
        })
    ...
    def test_replan(self, engine: GoalEngine):
        dag = asyncio.run(
            engine.decompose("Refactor module")
        )
