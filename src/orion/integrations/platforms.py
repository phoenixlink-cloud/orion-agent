...
def _build_platforms() -> dict[str, PlatformDef]:
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                ...
                PlatformCapability(
                    "speech_to_text", "Transcribe audio with Whisper", "transcribe this recording"
                ),
                PlatformCapability(
                    "text_to_speech", "Generate speech with OpenAI TTS", "read this aloud"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "chat", "Deep analysis with Claude", "review this PR for security issues"
                ),
                PlatformCapability(
                    "code_review", "Expert code review", "what could go wrong with this approach?"
                ),
                PlatformCapability(
                    "code_generation", "Generate code with Claude", "refactor this module"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                ...
                PlatformCapability(
                    "code_generation", "Generate code with Gemini", "write unit tests"
                ),
                PlatformCapability(
                    "google_drive",
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                ...
                PlatformCapability(
                    "code_generation", "Fast code generation", "generate boilerplate"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                ...
                PlatformCapability(
                    "code_generation", "Code with Codestral", "optimize this function"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                ...
                PlatformCapability(
                    "code_generation", "Code with DeepSeek/CodeLlama", "write a parser"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "chat", "Chat with any model", "compare GPT-4o vs Claude on this task"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            icon="🐙",
            auth_method=AuthMethod.CLI_TOOL,
            ...
            capabilities=[
                ...
                PlatformCapability(
                    ...
                    "find where auth is handled in my repos",
                ),
                PlatformCapability(
                    "create_issue", "Create GitHub issues", "create an issue for this bug"
                ),
                PlatformCapability(
                    "create_pr", "Create pull requests", "open a PR for these changes"
                ),
                PlatformCapability(
                    "read_issues", "Read and analyze issues", "what are the open issues?"
                ),
                PlatformCapability("read_pr", "Review pull requests", "summarize the latest PR"),
                PlatformCapability(
                    "read_file", "Read files from repos", "show me the README from my-repo"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "list_repos", "List your GitLab projects", "show my GitLab projects"
                ),
                PlatformCapability(
                    "create_issue", "Create GitLab issues", "create an issue for this"
                ),
                PlatformCapability(
                    "create_mr", "Create merge requests", "open an MR for these changes"
                ),
                PlatformCapability(
                    "read_pipeline", "Check CI/CD status", "what's the pipeline status?"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "run_code",
                    "Execute code safely in containers",
                    "run this Python script in a sandbox",
                ),
                PlatformCapability(
                    "test_code", "Run tests in isolation", "run pytest in a container"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "read_pages", "Read Notion pages", "read my project spec from Notion"
                ),
                PlatformCapability(
                    "search_docs", "Search Notion workspace", "find the design doc in Notion"
                ),
                PlatformCapability(
                    "create_page", "Create Notion pages", "create a meeting notes page"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "list_issues", "List Linear issues", "show my open issues in Linear"
                ),
                PlatformCapability(
                    "create_issue", "Create Linear issues", "create a bug report in Linear"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                ...
                PlatformCapability(
                    "create_issue", "Create Jira tickets", "create a story for this feature"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "send_message", "Send messages to Slack", "post the build status to #dev"
                ),
                PlatformCapability(
                    "send_snippet", "Share code snippets", "share this function in #code-review"
                ),
                PlatformCapability(
                    "read_channel", "Read channel history", "what was discussed in #architecture?"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "send_message", "Send messages to Discord", "post update to #general"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "text_to_speech", "Convert text to speech", "read this explanation aloud"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "speech_to_text", "Transcribe audio locally", "transcribe this recording"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "generate_image", "Generate images with Stable Diffusion", "create a UI mockup"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "generate_image", "Local image generation", "generate an icon locally"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "read_file", "Read files from Drive", "open my design doc from Drive"
                ),
                PlatformCapability(
                    "search_files", "Search Drive files", "find the API spec in Drive"
                ),
            ],
        )
    )
    ...
    _add(
        PlatformDef(
            ...
            capabilities=[
                PlatformCapability(
                    "read_file", "Read files from OneDrive", "open the spec from OneDrive"
                ),
            ],
        )
    )
...
class PlatformRegistry:
    ...
    def _refresh_status(self):
        ...
        for _pid, platform in self._platforms.items():
    ...
    def get(self, platform_id: str) -> PlatformDef | None:
    ...
    def get_platform_for_capability(self, capability: str) -> PlatformDef | None:
...
CATEGORY_LABELS = {
    "ai_models": {
        "label": "AI Models",
        "description": "Language models for coding, analysis, and generation",
        "icon": "🧠",
    },
    "developer_tools": {
        "label": "Developer Tools",
        "description": "Git, issue tracking, containers, and docs",
        "icon": "🛠️",
    },
    "messaging": {
        "label": "Messaging",
    ...
    "image": {
        ...
        "description": "Create images, diagrams, and mockups",
        "icon": "🎨",
    },
    "cloud_storage": {
        "label": "Cloud Storage",
        "description": "Access files from cloud drives",
        "icon": "☁️",
    },
}
