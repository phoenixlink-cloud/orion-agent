"""
Orion Agent — Deep Python Context Analysis (v6.4.0)
...
class PythonContext:
    ...
    def get_file_functions(self, rel_path: str) -> List[FunctionInfo]:
        ...
        return [f for f in self._functions.values()
                if f.file == rel_path and not f.is_method]
    ...
    def semantic_search(self, query: str, max_results: int = 10) -> List[Tuple[str, float]]:
        ...
        kind_weights = {"class": 5.0, "function": 3.0, "method": 2.0,
                        "variable": 1.0, "constant": 1.5}
    ...
    def get_context_for_file(self, rel_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "functions": [
                {"name": f.name, "args": f.args, "return_type": f.return_type}
                for f in functions
            ],
            "depends_on": deps,
    ...
    def _resolve_import(self, imp: ImportInfo) -> Optional[str]:
        module = imp.module
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                self._classes[key] = ClassInfo(
                    ...
                    bases=bases, methods=methods, decorators=decorators,
                    docstring=docstring, is_dataclass=is_dataclass,
    ...
    def _register_function(self, node, rel_path: str,
                           is_method: bool = False, parent_class: str = ""):
        ...
        self._functions[key] = FunctionInfo(
            name=node.name, file=rel_path, line=node.lineno,
            args=args, return_type=ret, calls=calls,
            decorators=decorators, docstring=docstring,
            is_method=is_method, parent_class=parent_class,
        )
        ...
        for node in ast.iter_child_nodes(tree):
            if isinstance(node, ast.ClassDef):
                ...
                for item in node.body:
                    if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        self._add_symbol(
                            item.name, "method", rel_path, item.lineno,
                            f"class:{node.name}"
                        )
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        ...
                        self._add_symbol(
                            target.id, kind, rel_path, node.lineno, "module"
                        )
