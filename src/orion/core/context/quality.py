...
def calculate_complexity(node: ast.AST) -> int:
    ...
    for child in ast.walk(node):
        if isinstance(
            child, (ast.If, ast.IfExp, ast.For, ast.While, ast.ExceptHandler, ast.With, ast.Assert)
        ):
            complexity += 1
...
class CodeQualityAnalyzer:
    ...
    def analyze(self, files: list[str] | None = None) -> QualityReport:
    ...
    def analyze_file(self, rel_path: str) -> FileHealth | None:
        fpath = self.workspace / rel_path
        return self._analyze_file(fpath, rel_path)

    def _analyze_file(self, fpath: Path, rel_path: str) -> FileHealth | None:
        try:
...
def get_file_quality(workspace_path: str, rel_path: str) -> FileHealth | None:
