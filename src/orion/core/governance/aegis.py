...
def check_aegis_gate(
    ...
    if intent.requires_action and mode not in ["pro", "project"]:
        violations.append(
            "AEGIS-2: File modifications require PRO or PROJECT mode. Use /mode pro or /mode project"
        )
    ...
    if action_type == "execution" and proposed_actions:
        for action in proposed_actions:
            ...
            if action_path and not _is_path_confined(action_path, workspace_path):
    ...
    if proposed_actions:
        for action in proposed_actions:
            ...
            if action_op == "RUN":
                if mode != "project":
                    violations.append(
                        "AEGIS-5: Command execution requires PROJECT mode. Use /mode project"
                    )
                else:
                    ...
                    if command:
                        ...
                        for op in forbidden_ops:
                            if op in command:
                                violations.append(
                                    f"AEGIS-5: Forbidden shell operator in command: {op}"
                                )
...
def validate_action_bundle(actions: list[dict], workspace_path: str) -> AegisResult:
    ...
    if not actions:
        return AegisResult(
            passed=True, violations=[], warnings=["No actions to validate"], action_type="execution"
        )
