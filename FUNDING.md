# Supporting Orion Development

## About This Project

Orion Agent is an open-source, multi-agent AI coding assistant developed and maintained by **Phoenix Link (Pty) Ltd**. The project is released under the AGPL-3.0 license, meaning it is free to use, modify, and distribute under the terms of that license.

## Why Support Orion?

Maintaining and developing Orion requires significant time and resources:

- **Core Development** -- New features, agent improvements, memory system enhancements
- **Security** -- Regular audits, vulnerability fixes, AEGIS governance hardening
- **Infrastructure** -- Testing environments, CI/CD, documentation hosting
- **Community Support** -- Issue triage, documentation, onboarding guides
- **Long-term Sustainability** -- Ensuring Orion remains actively maintained

## How to Contribute Financially

If Orion has benefited you in any way and you would like to see its continued development, please consider supporting the project by making a voluntary financial contribution.

### Make a Contribution

[![Support Orion Development](https://img.shields.io/badge/Support-Orion%20Development-blue?style=for-the-badge)](https://payf.st/t2pmo)

**Direct Link:** [https://payf.st/t2pmo](https://payf.st/t2pmo)

### Contribution Tiers

We offer suggested contribution levels, but any amount is appreciated:

| Tier | Amount (ZAR) | Recognition |
|------|--------------|-------------|
| Supporter | R50 | Our gratitude |
| Backer | R150 | Listed in SUPPORTERS.md (optional) |
| Sponsor | R250 | Listed in SUPPORTERS.md + README (optional) |
| Strategic Sponsor | R500+ | Logo in README (optional) |

*Recognition is optional -- let us know if you'd like to be listed.*

## Important Information

### This Is a Voluntary Contribution

- **This is NOT a purchase** -- You are not buying software, a license, or a product
- **This is NOT a transaction** -- You receive no additional rights beyond the AGPL-3.0 license
- **This is voluntary** -- There is no obligation to contribute
- **This supports development** -- Contributions fund ongoing maintenance and new features

### What You Get

As a contributor, you receive:

- The satisfaction of supporting open-source development
- Optional recognition in project documentation
- The same AGPL-3.0 license rights as everyone else

### What You Don't Get

Contributions do NOT provide:

- A commercial license (contact licensing@phoenixlink.co.za for that)
- Priority support (contact support@phoenixlink.co.za for that)
- Custom features (contact partnerships@phoenixlink.co.za for that)
- Any rights beyond the standard AGPL-3.0 license

## Other Ways to Support

Financial contributions aren't the only way to help:

- **Star the repository** -- Helps with visibility
- **Report bugs** -- Submit detailed issue reports
- **Improve documentation** -- Fix typos, add examples
- **Contribute code** -- See [CONTRIBUTING.md](./CONTRIBUTING.md)
- **Spread the word** -- Tell others about Orion

## Commercial Licensing

If you need to use Orion in a proprietary product or SaaS without AGPL-3.0 obligations, commercial licensing is available separately.

**Contact:** licensing@phoenixlink.co.za

See [LICENSE-ENTERPRISE.md](./LICENSE-ENTERPRISE.md) for details.

## Questions?

| Topic | Contact |
|-------|---------|
| Funding questions | funding@phoenixlink.co.za |
| Commercial licensing | licensing@phoenixlink.co.za |
| General inquiries | info@phoenixlink.co.za |

---

**Thank you for considering supporting Orion!**

*Phoenix Link (Pty) Ltd*
